...
public final class Consts {
    ...
    public static String PINGCLIENT_SUMMARY_STRING(int seqNRange, int survivedPackets, long minDelay, long delaySum, long maxDelay) {
        ...
        returnString += "Ping_Service.PingClient: " + seqNRange + " packets sent, " + survivedPackets + " packets received, ";
        int lossPercentage = round(((1 - ((float) survivedPackets / seqNRange)) * 100));
        returnString += lossPercentage + "% packet loss\n";
        returnString += "Ping_Service.PingClient: Round-trip (ms) min/avg/max = " + minDelay + "/" + round((float) delaySum * 100 / survivedPackets) / 100.0 + "/" + maxDelay;
        return returnString;
    }
}
